...
import interpreteExpresiones.TVALOR;
...
public class InterpretePullGUI extends InterpretePull {
	...
	public InterpretePullGUI(Visita e) {
		super(e);
		initializa();
	}
	...
	/**
	 * Transita por las diferentes etapas de la visita. Actualiza la etapa en la que estamos.
	 */
	@Override
	public void transita() {
		Continuacion c = this.etapa.continuaciones()[opcion];
		if(c.precondicion()==null) { //No tiene precondicion
			this.etapa = c.etapa();  
		}else{
			c.accion().acepta(this.iExpresiones);
		    if (this.iExpresiones.valor().tipo() == TVALOR.NUMERICO && this.iExpresiones.valor().valor()==0){
		        //No cumple la precondicion
		    }else{ 
		        this.etapa = c.etapa();  
		    }
		}
